...
import { PageHeader, EstadoBadge, ErrorBanner, SuccessBanner, InfoBanner } from "@/components/ui-bits";
...
function FichaProyecto() {
  ...
  const solicitudesGarantia = useStore((s) => s.solicitudesGarantia.filter((sg) => sg.proyectoId === id));
  ...
  if (!proyecto) {
    return (
      <div>
        ...
        <Link to="/seguimiento" className="inline-flex h-9 items-center gap-1.5 rounded-md border border-border bg-surface px-3 text-sm font-medium hover:bg-muted">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div className="rounded-lg border border-border bg-surface p-4">
          ...
          <dl className="space-y-1 text-sm">
            <div><dt className="text-xs text-muted-foreground">Cliente</dt><dd>{cliente?.nombre ?? "Eliminado"} ({cliente?.tipo})</dd></div>
            <div><dt className="text-xs text-muted-foreground">Tipo</dt><dd>{proyecto.tipo}</dd></div>
            <div><dt className="text-xs text-muted-foreground">Solicitud</dt><dd>{proyecto.fechaSolicitud}</dd></div>
            <div><dt className="text-xs text-muted-foreground">Ultima actualizacion</dt><dd>{proyecto.ultimaActualizacion}</dd></div>
          </dl>
          <div className="mt-3">
            <div className="mb-1 text-xs text-muted-foreground">Estado actual</div>
            <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${ESTADO_COLORS[proyecto.estado]}`}>{proyecto.estado}</span>
          </div>
          {puedeEditar && transiciones.length > 0 && (
            <div className="mt-3">
              <div className="mb-1 text-xs text-muted-foreground">Cambiar estado</div>
              {msgEstado && (msgEstado.ok ? <SuccessBanner>{msgEstado.txt}</SuccessBanner> : <ErrorBanner>{msgEstado.txt}</ErrorBanner>)}
        ...
        {/* Especificaciones */}
        <div className="rounded-lg border border-border bg-surface p-4">
          <h3 className="mb-2 text-sm font-semibold">Especificaciones ({proyecto.especificaciones.length})</h3>
              ...
              {(() => {
                ...
                return (
                  <ul className="space-y-1">
                    <li><span className="text-muted-foreground">Medidas:</span> {e.medidas}</li>
                    <li><span className="text-muted-foreground">Materiales:</span> {e.materiales}</li>
                    <li><span className="text-muted-foreground">Acabados:</span> {e.acabados}</li>
                    <li className="text-xs text-muted-foreground">v{e.version} - {e.actualizadoEn}</li>
                  </ul>
                );
              })()}
            </div>
        ...
        {/* Cotizaciones */}
        <div className="rounded-lg border border-border bg-surface p-4">
          <h3 className="mb-2 text-sm font-semibold">Cotizaciones ({proyecto.cotizaciones.length})</h3>
              ...
              {(() => {
                ...
                return (
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{c.descripcion} - {c.fecha}</div>
        ...
        <div className="rounded-lg border border-border bg-surface p-4">
          ...
          {ordenProduccion ? (
            <div className="text-sm">
              ...
              <div className="text-xs text-muted-foreground">Responsable: {ordenProduccion.responsable}</div>
              <Link to="/produccion/ordenes/$id" params={{ id: ordenProduccion.id }} className="mt-2 inline-flex text-xs text-primary hover:underline">Ver detalle</Link>
            </div>
          ) : proyecto.estado === "Aprobada" && (usuario.esAdmin || usuario.areas.includes("administrativa")) ? (
            <div>
              <p className="mb-2 text-sm text-muted-foreground">El proyecto esta aprobado. Puedes generar la orden de produccion.</p>
              <Link to="/administrativa/ordenes-produccion" className="text-xs text-primary hover:underline">Ir a crear orden</Link>
            </div>
          ) : (
            ...
            <ul className="space-y-1">
              {etapas.map((e) => (
                <li key={e.id} className="flex items-center justify-between text-sm">
                  <span>{e.nombre}</span>
                  <span className={`text-xs rounded-full px-1.5 py-0.5 ${e.estado === "completada" ? "bg-green-100 text-green-800" : e.estado === "en_curso" ? "bg-blue-100 text-blue-800" : "bg-gray-100 text-gray-800"}`}>{e.estado}</span>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-1">
              {transportes.map((t) => (
                <li key={t.id} className="text-sm">
                  <div>{t.fechaProgramada} - {t.estado}</div>
                  <div className="text-xs text-muted-foreground">{t.vehiculo} / {t.responsable}</div>
                </li>
              ))}
            </ul>
          </div>
            ...
            <ul className="space-y-1">
              {solicitudesGarantia.map((sg) => (
                <li key={sg.id} className="text-sm">
                  <Link to="/postventa/garantias/$id" params={{ id: sg.id }} className="text-primary hover:underline">{sg.fecha}</Link>
            ...
            <ul className="space-y-1">
              {proyecto.cambios.map((c) => (
                <li key={c.id} className="text-sm">
                  ...
                  <div className="text-xs text-muted-foreground">{c.fecha} - {c.responsable}</div>
                </li>
              ))}
            </ul>
          </div>
