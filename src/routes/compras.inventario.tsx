...
function InventarioPage() {
  ...
  const stocks = materiales.map((m) => ({ ...m, ...store.stockMaterial(m.id) }));
  ...
  const registrar = (e: React.FormEvent) => {
    ...
    store.registrarMovimiento({ materialId: form.materialId, tipo: "entrada", cantidad: Number(form.cantidad), fecha: new Date().toISOString().slice(0, 10), proveedorId: form.proveedorId || undefined, responsable: usuario.nombre, notas: form.notas || undefined });
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={puedeEditar ? <Button onClick={() => setShowForm((v) => !v)}><Plus className="h-4 w-4" /> {showForm ? "Cancelar" : "Registrar entrada"}</Button> : undefined}
      />
      {ok && <SuccessBanner>{ok}</SuccessBanner>}
      {showForm && (
        <form onSubmit={registrar} className="mb-4 space-y-4 rounded-lg border border-border bg-surface p-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Cantidad" required>
              <TextInput type="number" min={0.01} step="0.01" value={form.cantidad} onChange={(e) => setForm({ ...form, cantidad: Number(e.target.value) })} />
            </Field>
              ...
              <tr key={m.id} className="border-t border-border">
                <td className="px-3 py-2 font-medium">
                  <div className="flex items-center gap-1">
                    {m.disponible <= 0 && <AlertTriangle className="h-3.5 w-3.5 text-warning-foreground" title="Sin stock" />}
                    {m.nombre}
                  </div>
                </td>
                <td className="px-3 py-2 text-muted-foreground">{m.categoria}</td>
                <td className="px-3 py-2 text-muted-foreground">{m.unidad}</td>
                <td className={`px-3 py-2 text-right tabular-nums font-medium ${m.disponible <= 0 ? "text-destructive" : "text-success"}`}>{m.disponible}</td>
