    ...
    <div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-4">
        {ESTADOS.map((estado) => {
          ...
          return (
            <div key={estado} className="min-w-[220px] shrink-0">
                ...
                {cols.map((p) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => navigate({ to: "/comercial/proyectos/$id", params: { id: p.id } })}
                    >
                      <div className="text-sm font-medium">{p.codigo}</div>
                      <span className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-medium ${ESTADO_COLORS[p.estado]}`}>
                        {p.estado}
                      </span>
